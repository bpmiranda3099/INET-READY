...
try {
    ...
    // Fetch data from weatherdatahistory table
    $weatherQuery = "SELECT * FROM weatherdatahistory";
    $weatherStmt = $conn->prepare($weatherQuery);
    $weatherStmt->execute();
    $weatherData = $weatherStmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch data from weatherdataforecasthistory table
    $forecastQuery = "SELECT city, hi_day_one, hi_day_two, hi_day_three, hi_day_four, hi_day_five, hi_day_six, hi_day_seven, date_added, time_added FROM weatherdataforecasthistory";
    $forecastStmt = $conn->prepare($forecastQuery);
    $forecastStmt->execute();
    $forecastData = $forecastStmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch data from predictedhimetricshistory table
    $predictedQuery = "SELECT * FROM predictedhimetricshistory";
...
<body>
		...
		<div class="navbar-collapse collapse">
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="row">
						<div class="col">
							<h1 class="h3 mb-3"><strong>Weather</strong> History </h1>
						</div>
						<div class="col">
							...
							<div class="card flex-fill">
								<div class="card-header">
									<select id="tableSelect" class="form-control" onchange="showSelectedTable()">
										...
										<option value="weatherDataHistoryTable">Weather Data History</option>
										<option value="forecastDataHistoryTable">Heat Index Prediction History</option>
										<option value="predictedDataHistoryTable">Predicted Heat Index Metrics History</option>
									</select>
								</div>
								<div style="max-height: 1500px; auto; border: 1px solid #ddd; padding: 0px 10px 0px 10px;">
								<table id="weatherDataHistoryTable" class="table table-hover my-0" style="display: none;">
									<thead>
										<tr>
											<th>City</th>
											<th>Temperature</th>
										...
										<?php if (!empty($weatherData)): ?>
											<?php foreach ($weatherData as $data): ?>
												<tr>
													...
													<td><?= htmlspecialchars($data['inet_level']) ?></td>
								...
								<table id="forecastDataHistoryTable" class="table table-hover my-0" style="display: none;">
									<thead>
										<tr>
											<th>City</th>
								...
								<table id="predictedDataHistoryTable" class="table table-hover my-0" style="display: none;">
									<thead>
										<tr>
											<th>City</th>
		...
		$(document).ready(function() {
			...
			function initializeTable(tableId) {
				$('#' + tableId).DataTable({
					"pageLength": 10,
					"lengthMenu": [10, 15, 25],
					"columnDefs": tableId === 'weatherDataHistoryTable' ? [
						{
							"targets": '_all',
							"orderable": true
						},
			...
			$('#weatherDataHistoryTable, #forecastDataHistoryTable, #predictedDataHistoryTable').hide();
		});
	</script>
	...
	function showSelectedTable() {
		...
		// Hide all tables
		document.getElementById("weatherDataHistoryTable").style.display = "none";
		document.getElementById("forecastDataHistoryTable").style.display = "none";
		document.getElementById("predictedDataHistoryTable").style.display = "none";
